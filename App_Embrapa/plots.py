...
class plotsData():
    ...
    def melt_df(dataframe):
        ...
        dataframe.columns = vars + list(dataframe.columns[2:])

        df_check = dataframe.columns[2:]
        dataframe = dataframe[~dataframe[df_check].apply(lambda linha: (linha == 0).all(), axis = 1)]

        melted_df = dataframe.reset_index().melt(id_vars = vars, var_name = 'Year', value_name = 'Value')
    ...
    def bar(dataframe: pd.DataFrame, ):

        fig = px.bar(dataframe, x = 'Year', y = 'Value', color = 'Produto',)
        
        plot_html = fig.to_html(full_html = False, include_plotlyjs = 'cdn')
        return plot_html


    def scatter(dataframe: pd.DataFrame, ):

        fig = px.scatter(dataframe, x = 'Year', y = 'Value', color = 'Produto',)
        
        plot_html = fig.to_html(full_html = False, include_plotlyjs = 'cdn')
        return plot_html


    def bubble(dataframe: pd.DataFrame, ):
                
        fig = px.scatter(dataframe, x = 'Year', y = 'Value', color = 'Produto', size = 'Value',)
    ...
    def combine_df(dataframe: pd.DataFrame):
        ...
        df_ = dataframe[~dataframe[df_check].apply(lambda linha: (linha == 0).all(), axis = 1)]
